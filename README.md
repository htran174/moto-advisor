...
```bash
docker build -t rideready:latest .
```

Run the container (map host 5001 → container 5000):

```bash
docker run -p 5001:5000 --env-file .env rideready:latest
# Then open http://localhost:5001
```

---

### 3) GitHub Container Registry (CI Integration)

Your repository is configured so that **GitHub Actions** automatically:
- Builds the Docker image on push
- Publishes it to GitHub Container Registry at  
  `ghcr.io/htran174/rideready:latest`

You can manually pull and run it with:

```bash
docker pull ghcr.io/htran174/rideready:latest
docker run -p 5001:5000 --env-file .env ghcr.io/htran174/rideready:latest
```
...
- 💾 **Optional Chat Persistence** via SQLite or Flask sessions.  
- ⚡ **Async Image Caching** for Google API calls.  
- 🧩 **Frontend Polish** (animations, message bubbles).  
- 🔁 **Continuous Deployment (CD)** — Deploy container automatically to a hosting service (Render, AWS, or Railway).  
